...
assert_equal(marketplace.get("name"), "superplex-dev", "marketplace name")
assert_equal(
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "superplex"]
assert_equal(len(matching_plugins), 1, "superplex plugin entry count")
