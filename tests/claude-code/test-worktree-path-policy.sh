#!/usr/bin/env bash
# Regression check: Superplex should not route new worktrees through the old
...
assert_not_contains "$USING_SKILL" "~/.config/superplex/worktrees" "using-git-worktrees does not mention old global path"
...
assert_not_contains "$FINISHING_SKILL" "~/.config/superplex/worktrees" "finishing-a-development-branch does not treat old global path as owned"
...
assert_not_contains "$ROTOTILL_SPEC" "~/.config/superplex/worktrees" "rototill spec does not preserve old global path policy"
assert_not_contains "$ROTOTILL_PLAN" "~/.config/superplex/worktrees" "rototill plan does not preserve old global path policy"
