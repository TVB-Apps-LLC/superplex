# Porting Superplex to a New Harness

This guide explains how to add support for a new harness — an IDE, CLI, or
agent runner that isn't Claude Code — so that Superplex skills auto-trigger
...
## Part 1 — How Superplex works across harnesses

Superplex is the same content everywhere. What changes per harness is the thin
...
2. **Tool mapping (per-harness).** Each harness needs the action vocabulary
   ...
   `skills/using-superplex/references/<harness>-tools.md` and/or inline in the
...
3. **Bootstrap (per-harness).** At the start of every session, the full
   `skills/using-superplex/SKILL.md` is injected into the model's context,
...
A harness can support Superplex only if it can do all of the following. Check
...
If the only way to get Superplex in front of the model is for your human
...
1. The `using-superplex` bootstrap loads at session start, every session, with
...
model to describe its superplex. If the bootstrap injected, it knows it has
them. (OpenCode's install doc uses `opencode run --print-logs "hello" 2>&1 |
grep -i superplex` for the same goal via a different mechanism — log-grep
...
`using-superplex` skill's own description at session start). If more than one
...
`using-superplex/SKILL.md` and prints a JSON object whose **field name and
...
- Reference: `.opencode/plugins/superplex.js` (JavaScript) and
  `.pi/extensions/superplex.ts` (TypeScript). pi is the closest reference for
...
- Reference: `gemini-extension.json` (manifest, with `contextFileName`),
  ...
  reference), `skills/using-superplex/references/gemini-tools.md`.
...
- **Shape B:** the module the harness loads (e.g. `.<harness>/plugins/*.js`) plus
  ...
  - **Build/dependency check.** Decide how the harness loads your module:
    ...
    step? Superplex is zero-runtime-dependency. pi's `import type
...
`using-superplex` skill content (wrapped in `<EXTREMELY_IMPORTANT>` tags) plus
...
superplex… for all other skills use the Skill tool" preamble, escapes it, and
...
field/nesting, matcher strings — are the harness's contract, not Superplex',
...
pi's just says "do not try to load using-superplex again." If your harness has
...
*not* the user's own global file) pulls in two things: the `using-superplex`
skill and the harness's tool-mapping reference. `GEMINI.md`
does this with two `@`-includes (`@./skills/using-superplex/SKILL.md` and
`@./skills/using-superplex/references/<harness>-tools.md`); the harness loads
...
- **Shape A:** put it in `skills/using-superplex/references/<harness>-tools.md`.
...
- **Shape B:** the mapping is typically inlined into the bootstrap string you
  inject (see the `toolMapping` constant in `superplex.js`). pi keeps it in
...
`using-superplex/SKILL.md` tells the model to *never read skill files manually
...
2. **Native skill *discovery* but no `Skill` tool** (pi, Antigravity): the harness
   ...
   **For the bootstrap itself, prefer a declared context file (Part 6).** If the
   ...
   carries both the `using-superplex` content and the tool mapping. That is the
   ...
   **Fallback — the surfaced skill index.** If there's no context-file field but
   ...
   is the index, and `using-superplex`'s own surfaced description can be what
   ...
   - **It bootstraps *triggering*, not the *tool mapping*.** An injector prepends
     `<harness>-tools.md` alongside `using-superplex` every session. Here nothing
...
3. **No skill system at all:** there is nothing to register, and the *only*
   ...
   what it can't find: `using-superplex/SKILL.md` does **not** enumerate the
...
# 3. Smoke check: does the model know it has superplex?
...
tmux send-keys -t port-test 'What are your superplex?'; sleep 0.4; tmux send-keys -t port-test Enter
sleep 5
...
If the smoke check shows the model *doesn't* know it has superplex, the
...
|---|---|---|
| Native plugin marketplace | Claude Code | Register in `.claude-plugin/marketplace.json` — this repo doubles as its own marketplace (`superplex-dev`); users `/plugin marketplace add TVB-Apps-LLC/superplex` then `/plugin install superplex@superplex-dev`. |
| Local archive for portal upload | Codex | `scripts/package-codex-plugin.sh` stages the tracked plugin files (`.codex-plugin/`, `skills/`, etc.) into a rootless `superplex-VERSION.zip`/`.tar.gz` for manual upload to the Codex portal — see the script's header comment for why this replaced the old fork-sync-and-PR flow. Read its path allowlist so you ship the right tree. |
...
- **A plugin installer may silently strip *undeclared* files — so make the
  ...
  - **Ship a context file the manifest declares.** If the harness has a
    ...
    live `using-superplex/SKILL.md` + the tool mapping (wrapped in
    ...
    `✔ context : ANTIGRAVITY.md`, and a clean session reads `using-superplex`'s
    ...
    unrecognized.
  - **Otherwise lean on the installed `using-superplex` skill itself.** If the
    harness surfaces each installed skill's name + description at session start,
    the `using-superplex` description ("Use when starting any conversation…")
...
- **If no existing channel fits, you're standing up a new one.** None of the
  existing rows may match your harness. This fork does not maintain a
  fork-sync-and-open-a-PR script for any channel — the upstream Superpowers
  project's version of that (opening PRs against a marketplace fork it didn't
  own) is out of scope here. The closest analog for a "package it up and ship
  the archive" channel is `scripts/package-codex-plugin.sh`; start there if your
  channel needs something similar. And whenever you add a new per-harness
  directory, make sure it's excluded from any other harness's packaging step
  (e.g. the path allowlist in `scripts/package-codex-plugin.sh`) so your dotdir
  doesn't leak into their distributions.
...
- Fill in the PR template's **"New harness support"** section and paste the
  ...
  be closed.
- Superplex is a zero-dependency plugin. Don't add a third-party runtime
...
|---|---|---|---|---|---|
| Claude Code | `.claude-plugin/plugin.json` + `hooks/hooks.json` | shell hook → `hooks/session-start` (`hookSpecificOutput.additionalContext`) | native `Skill` tool; `references/claude-code-tools.md` | `tests/hooks/` | marketplace (`superplex-dev`, this repo) |
| Codex | `.codex-plugin/plugin.json` (declares empty `hooks`) | native skill discovery (no session-start hook) | `references/codex-tools.md` | `tests/codex/` | portal archive (`scripts/package-codex-plugin.sh`) |
| Cursor | `.cursor-plugin/plugin.json` + `hooks/hooks-cursor.json` | shell hook → `hooks/session-start` (`additional_context`) | `references/claude-code-tools.md` | `tests/hooks/` | hand-authored |
| Copilot CLI | (shares Claude Code hook path; `COPILOT_CLI` env) | shell hook → `hooks/session-start` (`additionalContext`) | `references/copilot-tools.md` | `tests/hooks/` | — |
| Gemini CLI | `gemini-extension.json` + `GEMINI.md` | instructions file `@`-includes bootstrap + mapping | `references/gemini-tools.md` | — | `gemini extensions install` |
| Kimi Code | `.kimi-plugin/plugin.json` | manifest `sessionStart.skill` loads `using-superplex` | inline `skillInstructions` in manifest | `tests/kimi/` | `/plugins install` GitHub URL |
| OpenCode | `.opencode/plugins/superplex.js` (declared via root `package.json` `main`) | in-process: `config` hook registers skills dir; `experimental.chat.messages.transform` injects user message | inline in `superplex.js` | `tests/opencode/` | `opencode.json` plugin git URL |
| pi | `.pi/extensions/superplex.ts` | in-process: `resources_discover` registers skills; `context` event injects user message; lifecycle-flag + compaction-aware | `piToolMapping()` inline **and** `references/pi-tools.md` | `tests/pi/` | repo-root `package.json` fields |

## Appendix B — Gotchas that have bitten porters

- **Opt-in isn't a port.** If your human partner has to do anything per session
  to get Superplex, the acceptance test fails. Re-read Part 2.
