...
main() {
    ...
    if [[ "$dir" == "$repo/.superplex/sdd" ]]; then
        pass "prints <repo-root>/.superplex/sdd"
    else
        fail "prints <repo-root>/.superplex/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.superplex/sdd/.gitignore" && "$(cat "$repo/.superplex/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.superplex/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.superplex/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.superplex/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.superplex/sdd" && "$wt_dir" != "$dir" ]]; then
