#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the superplex plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/superplex.js"
...
skill_count=$(find "$SUPERPLEX_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $SUPERPLEX_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-superplex skill exists (critical for bootstrap)
echo "Test 3: Checking using-superplex skill (required for bootstrap)..."
if [ -f "$SUPERPLEX_SKILLS_DIR/using-superplex/SKILL.md" ]; then
    echo "  [PASS] using-superplex skill exists"
else
    echo "  [FAIL] using-superplex skill not found (required for bootstrap)"
    exit 1
fi

# Test 4: Verify plugin JavaScript syntax (basic check)
echo "Test 4: Checking plugin JavaScript syntax..."
if node --check "$SUPERPLEX_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/superplex/' "$SUPERPLEX_PLUGIN_FILE"; then
