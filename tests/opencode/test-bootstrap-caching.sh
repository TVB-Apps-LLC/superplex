...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERPLEX_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$SUPERPLEX_SKILLS_DIR/using-superplex/SKILL.md" "$TEST_HOME/using-superplex.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$SUPERPLEX_PLUGIN_FILE" missing
}
