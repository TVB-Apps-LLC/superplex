...
const extensionPath = resolve(repoRoot, '.pi/extensions/superplex.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-superplex/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'superplex');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/superplex.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have superplex/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have superplex/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have superplex/);
