...
# 1. Create in superplex location (lowest priority)
mkdir -p "$SUPERPLEX_SKILLS_DIR/priority-test"
cat > "$SUPERPLEX_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Superplex version of priority test skill
---
# Priority Test Skill (Superplex Version)

This is the SUPERPLEX version of the priority test skill.

PRIORITY_MARKER_SUPERPLEX_VERSION
EOF
...
if [ -f "$SUPERPLEX_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Superplex version exists"
else
    echo "  [FAIL] Superplex version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled superplex priority
echo ""
echo "Test 2: Documenting personal vs superplex priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superplex version instead of personal version"

# Test 3: Document project vs bundled superplex priority
echo ""
echo "Test 3: Documenting project vs personal/superplex priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled superplex version instead of project version"

# Test 4: Test a non-colliding bundled superplex skill is still available
echo ""
echo "Test 4: Testing non-colliding superplex skill remains available..."

mkdir -p "$SUPERPLEX_SKILLS_DIR/superplex-only-test"
cat > "$SUPERPLEX_SKILLS_DIR/superplex-only-test/SKILL.md" <<'EOF'
---
name: superplex-only-test
description: Superplex-only priority test skill
---
# Superplex Only Test Skill

PRIORITY_MARKER_SUPERPLEX_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"superplex-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_SUPERPLEX_ONLY_VERSION" "Non-colliding superplex skill is still registered"
